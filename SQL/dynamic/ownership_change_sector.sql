...
running_total AS (
    SELECT year, sector, 
    ROUND(SUM(difference) OVER (PARTITION BY sector ORDER BY year) * 100, 2) AS cumulative_percent_change_of_ownership
    FROM difference
    WHERE difference IS NOT NULL
)

SELECT sector AS "Sector", cumulative_percent_change_of_ownership
FROM running_total
